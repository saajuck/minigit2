...
# convention expects it: src-tauri/binaries/minigit2-server-<target-triple>.exe
#
# Runs via Git Bash, which ships with every GitHub Actions windows-latest runner (and most
# Windows Git installs) — the script is otherwise identical to packaging/linux/build-sidecar.sh,
# since Node's SEA tooling and postject are already cross-platform. The only real differences are
# the target triple default and the .exe suffix Tauri's sidecar lookup expects on Windows;
...
OUT_BIN="$OUT_DIR/minigit2-server-$TARGET_TRIPLE.exe"
WORK_DIR="$(mktemp -d)"
